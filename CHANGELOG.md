...
- Webhook requests whose body cannot be decoded as UTF-8 or parsed as JSON now receive an HTTP 400 response instead of being processed with an empty payload. Previously the handler silently fell through and fired a spurious "Unknown alert" into all category sensors. ([#173])
