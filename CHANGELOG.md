...
- A re-authentication that succeeds but is immediately followed by another 401 on the retried fetch now raises `ConfigEntryAuthFailed` (prompting HA re-auth) instead of escaping as an unhandled exception. ([#122])
