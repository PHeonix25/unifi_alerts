...
- Always run `make doc-check` first for translation parity, then `make check` before committing. Never commit broken code.
...
2. Run `make doc-check` (translation parity + docs checks), then `make check`.
3. Build and test command definitions are canonical in [`AGENTS.md` > Build & test commands](AGENTS.md#build--test-commands). Run `make help` for the full local target list.
...
Single source: [`docs/DEVELOPING.md` > Doc-only changes (lighter path)](docs/DEVELOPING.md#doc-only-changes-lighter-path).
