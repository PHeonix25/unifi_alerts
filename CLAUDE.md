...
scripts/
  ...
  setup-labels.sh                 # one-shot script that creates the non-default labels referenced in `.github/release.yml` (`security`, `tests`, `ci`, `github-actions`, `dependencies`). Run once per fork via `./scripts/setup-labels.sh`. Idempotent — existing labels skipped. The `bug`, `enhancement`, and `documentation` labels are GitHub defaults; the rest do not exist on a fresh fork and the categories file is inert without them.
