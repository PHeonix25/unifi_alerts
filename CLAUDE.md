...
| `AGENTS.md` | Agent persona definitions for GitHub Copilot and third-party tools. Read when working in a context that loads this file automatically. |
...
- At the end of the day, make sure there are no commits outstanding and no changes locally that need to be pushed (`git status` should show a clean working tree). This ensures a clean slate for the next session.
