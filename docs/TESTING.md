...
- `tests/conftest.py` applies two Windows-only workarounds. Both are no-ops on Linux/macOS.
  - **Event-loop policy.** It installs `asyncio.WindowsSelectorEventLoopPolicy` at module load time. `aiodns` (pulled in transitively via `aiohttp`/HA) raises `RuntimeError: aiodns needs a SelectorEventLoop on Windows` against Python's default `ProactorEventLoop`. See [aiodns#86](https://github.com/saghul/aiodns/issues/86).
