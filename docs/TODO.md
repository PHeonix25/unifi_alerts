...
- **`open_count` stale on webhook path** (`coordinator.py`): `push_alert()` updates `is_alerting` and `alert_count` but `open_count` only refreshes on the next poll. Consider an optimistic increment with poll-time correction.
...
- **Troubleshooting / FAQ section**: consolidate scattered notes (local-only webhooks, self-signed certs, "why is `open_count` so high?", API-key generation paths, cloud-access failures). Two field-confirmed scenarios to include:
