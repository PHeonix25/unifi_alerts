...
- **`open_count` lags on webhook path; IPS events may be in a separate polling endpoint** (`coordinator.py`, `unifi_client.py`): `push_alert()` never updates `open_count`; only polling does. Additionally, IPS/threat events may live at `/proxy/network/api/s/{site}/stat/ips/event` rather than `/list/alarm` (the unpoller reverse-engineering library defines them as distinct endpoints). If IPS events are absent from `/list/alarm`, the `security_threat` `open_count` can never be non-zero via polling regardless of watermark. Field-confirmed: `alert_count=11`, `open_count=0` after 45+ minutes. Caveat: production screenshots also show polling re-asserting `is_alerting` with an IPS-type message 63 s after auto-clear, which may indicate that UniFi Network 9.x merged IPS events into `/list/alarm`; this is firmware-version dependent and needs investigation. Fix part 1: optimistic increment in `push_alert()` when `alert.received_at > state.last_cleared_at`. Fix part 2: investigate `/stat/ips/event`; if needed, add it as a second polling path for `security_threat`. Pair both parts with regression tests.
