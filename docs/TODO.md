...
- **`open_count` lags on webhook path** (`coordinator.py`): `push_alert()` updates `is_alerting` and `alert_count` but never touches `open_count`; only polling sets it. For up to one poll interval (default 60 s) after a webhook, the binary sensor shows Problem while Open Count shows 0. Field-confirmed: `alert_count=11`, `open_count=0`. Disproven hypotheses (recorded so we don't re-investigate): (a) UniFi auto-archives IPS alarms; research shows there is no auto-archive in Network 9.x; (b) IPS events live at a separate `/stat/ips/event` endpoint; direct probing of a UCG-Ultra returned `api.err.NotFound`, and `/list/alarm` confirmed to contain `EVT_IPS_IpsAlert` records with full payload (`signature`, `srcip`, `dest_port`, etc.). The actual cause is simply that `push_alert()` doesn't increment `open_count`; the next poll should reconcile but only if today's webhook-delivered alarms appear in `/list/alarm` (an open question; needs a controlled test searching the response for today's date). Fix: optimistic increment in `push_alert()` when `alert.received_at > state.last_cleared_at`, with poll-time correction (clamp down if poll comes back lower). Pair with regression tests.
