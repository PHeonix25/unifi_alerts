...
- **`open_count` never lifts above 0 on the webhook path** (`coordinator.py`): `push_alert()` updates `is_alerting` and `alert_count` but never touches `open_count`; only polling sets it. UniFi auto-archives IPS/threat alarms faster than the default 60s poll catches them, so polling consistently sees zero matching alarms and `open_count` stays at 0 indefinitely even with real webhook deliveries. Field-confirmed: `alert_count=11`, `last_cleared_at` 45 minutes prior, `open_count=0`. The "Total Open Alerts" rollup sensor is therefore unusable for short-lived alarm categories. Fix: optimistic increment in `push_alert()` + poll-time correction (clamp down if poll comes back lower). Pair with a regression test in `test_coordinator.py`.
