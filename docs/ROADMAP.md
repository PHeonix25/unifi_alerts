...
- [x] **`UniFiClient.close()` silently swallows logout errors** — fixed in v1.5.0 Bundle 1 (2026-05-04). See v1.5.0 Documentation section.
...
- [x] **Config flow creates bare `aiohttp.ClientSession`** — fixed in v1.5.0 Bundle 2 (2026-05-04). See v1.5.0 Authentication / credential safety section.
- [x] **Credential fragments may leak in `__init__.py` exception messages** — fixed in v1.5.0 Bundle 1 (2026-05-04). See v1.5.0 Authentication / credential safety section.
...
- [x] **Config flow bare `aiohttp.ClientSession` → `async_get_clientsession`** — fixed in v1.5.0 Bundle 2 (2026-05-04): all three sites (`async_step_user`, `async_step_reauth_confirm`, `async_step_credentials`) now use `async_get_clientsession(self.hass, verify_ssl=...)` so the user's SSL setting and HA's proxy / connection-pool config are honoured. `import aiohttp` dropped. Regression test `test_user_step_uses_ha_clientsession_with_user_verify_ssl` asserts the function is called with the user's `verify_ssl` value.
- [x] **Credential fragments in `__init__.py` exception messages** — fixed in v1.5.0 Bundle 1 (2026-05-04): all three `ConfigEntryAuthFailed` / `ConfigEntryNotReady` raises and the two paired `_LOGGER.error` calls now surface `type(err).__name__` only. Regression tests in `test_init.py` assert the underlying `str(err)` content does not appear in the raised exception.
...
- [x] **Document secret rotation threat model in `SECURITY.md`** — fixed in v1.5.0 Bundle 1 (2026-05-04): new "Webhook secret rotation" section in `SECURITY.md` explains that rotation replaces the bearer token but reuses the URL path; the constant-time check rejects old tokens but URL-path revocation requires deleting and re-adding the entry. `# WHY:` comment in `config_flow.py` near the rotation branch points at the section.
- [x] **Close() logout errors** — fixed in v1.5.0 Bundle 1 (2026-05-04): replaced `contextlib.suppress(Exception)` with try/except that logs `_LOGGER.warning("UniFi logout failed: %s", type(err).__name__)`. Best-effort behaviour preserved (no propagation). `tests/unit/test_unifi_client.py::TestClose` covers both warning emission and non-propagation.
