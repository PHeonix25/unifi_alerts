...
5. `services.yaml` service and field names/descriptions are mirrored in `strings.json`'s `services` section, so the frontend can override the YAML defaults with localized text.
...
| Config flow string literals | Read every `async_show_form`, `async_create_entry`, `async_abort`, and `errors[...]` assignment in `config_flow.py` | Gap found and fixed (see below). |
| Config flow error keys vs `strings.json` | Cross-checked every `errors["base"]` / `errors[<field>]` value against `config.error` and `options.error` in `strings.json` | Gap found and fixed (see below). |
| Config flow abort reasons vs `strings.json` | Cross-checked every `async_abort(reason=...)` value against `config.abort` | Gap found and fixed (see below). |
...
| `strings.json` / `translations/en.json` byte identity | `scripts/check_translations.py` | Pass, after the fixes below were applied identically to both files. |

### Gaps found and fixed

Two real gaps were found, both in `config_flow.py`'s options (reconfigure) flow and SSDP discovery path. Both were fixed by adding the missing keys to `strings.json` and `translations/en.json` identically:

1. **Options flow error keys were missing.** The reconfigure flow's credentials and categories steps set `errors["base"]` / `errors[<field>]` to `cannot_connect`, `invalid_auth`, `invalid_ssl_cert`, `invalid_url_scheme`, and `at_least_one_category`, but `strings.json`'s `options.error` section only defined `invalid_site`. Home Assistant translates config-flow and options-flow errors from separate namespaces (`config.error` and `options.error` respectively); there is no fallback from one to the other. Without this fix, a user hitting any of these five error conditions while reconfiguring an existing entry would see the raw untranslated key instead of a readable message. Fixed by adding all five keys to `options.error`, using the same wording as the equivalent `config.error` entries.
2. **A config-flow abort reason was missing.** `async_step_ssdp` aborts with `reason="cannot_connect"` when the discovered device's SSDP location URL has no parseable hostname, but `strings.json`'s `config.abort` section only defined `already_configured` and `reauth_successful`. Fixed by adding a `cannot_connect` entry under `config.abort` describing the actual failure (the controller address could not be determined from the discovery information).

No other gaps were found. `strings.json` and `translations/en.json` remain byte-identical after the fix, verified with `diff`.
...
5. Open a PR with just the new `translations/<lang>.json` file. Since this only affects a new file with no change to existing behavior, it does not require a `CHANGELOG.md` entry unless the maintainer requests one; apply the `documentation` label if no other CI-recognised label fits.
